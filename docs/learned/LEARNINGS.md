...
- Model fallback is launch-time only via `has_provider()` -- no mid-run failover. Credential checks happen when the selected provider is created/called so child runs can inherit request-scoped auth (`api_key_override` / `get_api_key`).
